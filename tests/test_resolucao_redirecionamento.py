"""Testes BDD — GET /{codigo}: resolução e redirecionamento (t001).

Cobre os 4 cenários da US-002:
  A — Código existente  → HTTP 302 com Location correto
  B — Código inexistente → HTTP 404, sem redirecionamento
  C — Formato inválido   → HTTP 404, sem consulta ao storage
...
# ---------------------------------------------------------------------------
# Cenário B — Código inexistente: HTTP 404
# ---------------------------------------------------------------------------
...
class TestCenarioB:
    ...
    def test_corpo_tem_detail(self):
        resp = client.get("/abcdef")
        assert "detail" in resp.json()

    def test_mensagem_de_detalhe(self):
        resp = client.get("/abcdef")
...
# ---------------------------------------------------------------------------
# Cenário C — Formato inválido: HTTP 404, sem consulta ao storage
# ---------------------------------------------------------------------------
...
class TestCenarioC:
    ...
    @pytest.mark.parametrize("codigo_invalido", [
        "",           # vazio — tratado como rota diferente pelo FastAPI, mas testamos is_valid_code
        "abc",        # curto demais
...
class TestCenarioD:
    ...
    def test_acesso_concorrente_acesso_count_consistente(self):
        ...
        # O contador deve ser exatamente n_threads (sem perdas por race condition
        # no GIL do CPython com dict puro).
