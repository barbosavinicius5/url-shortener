...
def concurrent_engine_and_session():
    ...
    app.dependency_overrides[get_db] = __import__(
        "tests.conftest", fromlist=["_get_test_db"]
    )._get_test_db
    Base.metadata.drop_all(bind=eng)
    eng.dispose()
...
class TestCenarioC:
    ...
    def test_c2_cliques_de_codigos_diferentes_sao_independentes(
        self, concurrent_engine_and_session
    ):
